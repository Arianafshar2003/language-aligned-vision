# ridge_regression_siglip2.py
"""
Voxelwise ridge regression encoding model for SigLIP2 PCA features.

Usage:
  python ridge_regression_siglip2.py <subject_number>

Output:
  best_alphas_siglip2.npy
  Y_pred_siglip2.npy
"""
...
def process_subject(subj):
    """
    Fit voxelwise ridge regression for SigLIP2 features.
    ...
    FEATURE_FILE = output_dir / "features_siglip2_large_pca.npy"
    BETA_FILE = output_dir / f"betas_averaged_final_{subj}.npy"
    ALPHAS_FILE = output_dir / "best_alphas_siglip2.npy"
    PRED_FILE = output_dir / "Y_pred_siglip2.npy"

    # Validate inputs
    if not FEATURE_FILE.exists():
        print(f"Error: PCA features not found: {FEATURE_FILE}")
        print("   Run pca_siglip2.py first.")
        return
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python ridge_regression_siglip2.py <subject_number>")
        sys.exit(1)
