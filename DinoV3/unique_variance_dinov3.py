# unique_variance_dinov3.py
"""
Variance partitioning analysis for DINOv3 comparisons.

Supports four comparison directions:
  desc_over_dino    -- Unique variance of CLIP descriptions beyond DINOv3
  dino_over_desc    -- Unique variance of DINOv3 beyond CLIP descriptions
  siglip_over_dino  -- Unique variance of SigLIP2 beyond DINOv3
  dino_over_siglip  -- Unique variance of DINOv3 beyond SigLIP2
  all               -- Run all four directions

For each comparison:
1. Concatenates RAW (non-PCA) features from both models.
...
Usage:
  python unique_variance_dinov3.py <subject_number> <direction>

Example:
  python unique_variance_dinov3.py 1 desc_over_dino
  python unique_variance_dinov3.py 1 all
"""
...
RAW_FEATURES = {
    ...
    "dinov3":       "features_dinov3_large2.npy",
...
# Comparison definitions: (feature_A, feature_B, joint_output_suffix)
COMPARISONS = {
    "desc_over_dino":   ("descriptions", "dinov3",  "descriptionVsDino3"),
    "dino_over_desc":   ("dinov3",       "descriptions", "Dino3VsDescription"),
    "siglip_over_dino": ("siglip2",      "dinov3",  "SiglipVsDino3"),
    "dino_over_siglip": ("dinov3",       "siglip2", "Dino3VsSiglip"),
}
...
def run_unique_variance(output_dir, feature_A, feature_B, Y, split_point,
    ...
    2. Joint PCA (97% variance)
    3. Fit ridge on joint space
    4. Compute R2 values and Unique_A
    5. Bootstrap test with FDR correction
    6. Save results
    ...
    Args:
        ...
        Y: Full beta matrix (n_images x n_voxels)
        split_point: Index for 80/20 train/test split
        output_suffix: String for output filenames
    """
    ...
    print("    [4] Loading single-model predictions...")
    ...
    print("    [5] Computing R2 scores (A, B, AB)...")
    ...
    np.save(str(output_dir / f"R2_A_sub{subj:02d}_{output_suffix}.npy"), R2_A.cpu().numpy())
    np.save(str(output_dir / f"R2_B_sub{subj:02d}_{output_suffix}.npy"), R2_B.cpu().numpy())
    np.save(str(output_dir / f"R2_AB_sub{subj:02d}_{output_suffix}.npy"), R2_AB.cpu().numpy())
    np.save(str(output_dir / f"Unique_A_sub{subj:02d}_{output_suffix}.npy"), Unique_A.cpu().numpy())
    np.save(str(output_dir / f"mask_significant_A_sub{subj:02d}_{output_suffix}.npy"), mask)
...
def process_subject(subj, direction):
    ...
    if direction == "all":
        comparisons_to_run = list(COMPARISONS.items())
    else:
        comparisons_to_run = [(direction, COMPARISONS[direction])]

    for comp_name, (feat_A, feat_B, suffix) in comparisons_to_run:
        print(f"\n{'='*50}")
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python unique_variance_dinov3.py <subject> <direction>")
        print("Direction options:")
        for key, (a, b, _) in COMPARISONS.items():
            print(f"  {key:<20s} -- Unique variance of {a} beyond {b}")
        print("  all                  -- Run all four comparisons")
        sys.exit(1)
    ...
    valid_directions = list(COMPARISONS.keys()) + ["all"]
    if direction not in valid_directions:
        print(f"Error: Invalid direction '{direction}'")
        print("Choose from:", ", ".join(valid_directions))
        sys.exit(1)
